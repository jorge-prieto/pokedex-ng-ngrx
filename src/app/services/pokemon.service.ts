...
export class PokemonService {
  ...
  getPokemons(url: string): Observable<PokemonDataList> {
  ...
  getPokemon(url: string): Observable<Pokemon> {
    return this.http.get(url).pipe(
      switchMap((result: PokemonData) => {
        result.url = url;
        return this.http.get(result.species.url).pipe(
          map((res: PokemonSpecie) => {
  ...
  getPokemonDetails(data: PokemonData, specie: PokemonSpecie): Pokemon {
    ...
    const desc = specie.flavor_text_entries.find(
