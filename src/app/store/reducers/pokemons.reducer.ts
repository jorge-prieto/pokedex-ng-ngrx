...
const pokemonsReducer = createReducer(
  initialState,
  on(PokemonsActions.loadPokemonsSuccess, (state, { nextUrl, pokemons }) => {
    return adapter.setAll(pokemons, { ...state, nextUrl });
  }),
  on(PokemonsActions.addPokemonsSuccess, (state, { nextUrl, pokemons }) => {
    localStorage.setItem('nextUrl', nextUrl);
    return adapter.addMany(pokemons, { ...state, nextUrl });
  }),
  on(PokemonsActions.setFavoritePokemonSuccess, (state, { update }) => {
    if (update.id === state.toShow?.id) {
      return adapter.updateOne(update, {
        ...state,
        toShow: {
          ...state.toShow,
          isFavorite: update.changes.isFavorite,
        },
      });
    }
  ...
  on(PokemonsActions.selectPokemon),
  on(
  ...
  on(PokemonsActions.comparePokemons, (state: PokemonsState) => {
    return { ...state, comparing: true };
  }),
  on(PokemonsActions.closeModal, (state: PokemonsState) => {
    return {
      ...state,
      comparing: false,
  ...
  on(PokemonsActions.setError, (state: PokemonsState, { msg }) => {
    return {
      ...state,
      error: { msg },
    };
