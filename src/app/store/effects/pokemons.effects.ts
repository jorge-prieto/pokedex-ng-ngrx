...
import {
  getNextUrl,
  selectAllFavoritePokemons,
  selectAllPokemons,
} from '../selectors/pokemons.selectors';
...
export class PokemonsEffects {
  ...
  loadPokemons$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([, { nextUrl: url }]) => {
        return this.pokemonService.getPokemons(url).pipe(
          map(({ nextUrl, pokemons }) => {
            return PokemonsActions.addPokemonsSuccess({
              ...
              pokemons: pokemons.map((pokemon) => {
                return {
                  ...pokemon,
                  isFavorite: environment.initialPokemons.includes(
                    pokemon.name
                  ),
                };
              }),
    ...
    this.actions$.pipe(
      ...
      map(([pokemon, favoritePokemons]) => {
        if (favoritePokemons.length >= 5 && pokemon.value) {
          return PokemonsActions.setError({
            msg: 'Error: Maximum number of favorite pokemons has been reached',
          });
        } else {
  ...
  selectPokemon$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ pokemon }) => {
        return this.pokemonService.getPokemon(pokemon.url).pipe(
          map((result) => {
  ...
  saveStorage$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(
          PokemonsActions.addPokemonsSuccess,
          PokemonsActions.setFavoritePokemonSuccess
        ),
